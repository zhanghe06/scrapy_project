...
class Crawl58ServiceBanjiaV4Spider(scrapy.Spider):
    name = "crawl58_service_banjia_v4"
    ...
    def parse_list(self, response):
        ...
        for tr in trs:
            ...
            if service_link:
                # 公司名称
                company_name = tr.xpath('.//p[contains(@class, "seller")]//a[@class="u"]/text()').extract_first(default='').strip()
                # 抓取详情页面
    ...
    def parse_detail(self, response):
        ...
        service_pub_date = response.xpath('//div[@id="index_show"]//li[@class="time"]/text()').extract_first(default='').strip()
        service_district = ' '.join([item.strip('') for item in response.xpath('//div[contains(@class, "quyuline")]/a/text()').extract()])
        ...
        company_address_re_list = response.xpath('//div[@class="userinfo"]/ul[@class="uinfolist"]/li').re(self.company_address_re_compile)
        company_address = company_address_re_list[0].strip() if company_address_re_list else ''
        company_reg_date_re_list = response.xpath('//div[@class="userinfo"]/ul[@class="uinfolist"]/li').re(self.company_reg_date_re_compile)
        ...
        # 认证状态（个人、企业）
        verified_personal = int(response.xpath('//span[@class="ico-rzv-o"]/@title').extract_first(default='').strip() == u'个人身份已认证')
        verified_company = int(response.xpath('//span[@class="ico-rzv-b"]/@title').extract_first(default='').strip() == u'企业营业执照已认证')
...
# scrapy genspider crawl58_service_banjia_v4 58.com
# scrapy crawl crawl58_service_banjia_v4 -s JOBDIR=crawls/app/crawl58_service_banjia_v4


# 会员年份， 联系方式 都是 ajax 加载的
...
# 统计标题重复的数据
# select service_title, count(*) as c from service group by service_title order by c desc;
#
# select count(distinct service_title) from service;
#
# 统计分类
# select fetch_city_code, fetch_cate_code, count(*) as c from service group by fetch_city_code, fetch_cate_code order by c desc;
